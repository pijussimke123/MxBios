...
struct xen_mxbios_info {
    char signature[14]; /* XenHVMmxbios\0 */
...
static void validate_info(struct xen_mxbios_info *t)
{
    if ( memcmp(t->signature, "XenHVMmxbios", 14) )
        panic("Bad Xen info signature\n");

    if ( t->length < sizeof(struct xen_mxbios_info) )
...
{
    struct xen_mxbios_info *info = (void *)INFO_PHYSICAL_ADDRESS;
